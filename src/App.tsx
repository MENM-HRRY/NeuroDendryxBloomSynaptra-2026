...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isCmdOrCtrl && e.key.toLowerCase() === 'e') { e.preventDefault(); setLanguage('en'); }
      if (isCmdOrCtrl && e.key.toLowerCase() === 's') { e.preventDefault(); setLanguage('es'); }
      if (isCmdOrCtrl && e.key.toLowerCase() === 'p') { e.preventDefault(); searchInputRef.current?.focus(); }
      if (isCmdOrCtrl && e.key.toLowerCase() === 'l') { e.preventDefault(); setSearchQuery(''); }
    };
  ...
  const handleUpload = async () => {
    setError(null);
    if (!englishFile) { setError("English file is mandatory."); return; }
    if (hasSpanish && !spanishFile) { setError("Spanish file is enabled but not selected."); return; }
    try {
      const enData = await parseFile(englishFile, hasExplanation);
      let esData: any[] = [];
      if (hasSpanish && spanishFile) { esData = await parseFile(spanishFile, hasExplanation); }
      if (enData.length === 0) throw new Error("No valid questions found in English file.");
      const combined: Question[] = enData.map((enQ, index) => {
        const esQ = esData.find(q => q.id === enQ.id) || esData[index];
        return {
          id: enQ.id,
          en: { pregunta: enQ.pregunta, respuesta: enQ.respuesta, explicacion: enQ.explicacion },
          es: hasSpanish && esQ
            ? { pregunta: esQ.pregunta, respuesta: esQ.respuesta, explicacion: esQ.explicacion }
            : { pregunta: enQ.pregunta, respuesta: enQ.respuesta, explicacion: enQ.explicacion }
        };
      });
                ...
                <AnimatePresence>
                  {!customQuestions && (
                    <motion.button
                      ...
                      initial={{ opacity: 0, x: 50, y: -80, rotate: -45 }}
                      animate={{ opacity: 1, x: 0,  y: 0,   rotate: 0   }}
                      exit={{    opacity: 0, x: 50, y: -80, rotate: -45 }}
                ...
                >
                  ...
                  <div className="p-5 space-y-4">
                    <div className="space-y-1">
                      ...
                      <div className={`p-3 rounded-lg transition-colors duration-300 ${isFiltered ? 'bg-[#172554] text-blue-100 border border-blue-900/50' : 'bg-transparent text-gray-300'}`}>
                        <p className="text-base leading-relaxed">
                          {highlight(content.respuesta)}
                        </p>
                      </div>
                    </div>
                    ...
                    {content.explicacion && (
                      <div className="space-y-1">
                        ...
                        <div className={`p-3 rounded-lg transition-colors duration-300 ${isFiltered ? 'bg-[#064e3b] text-green-100 border border-green-900/50' : 'bg-transparent text-gray-400 italic'}`}>
                          <p className="text-sm leading-relaxed">
                            {highlight(content.explicacion)}
                          </p>
                        </div>
                      </div>
                    )}
          ...
          {filteredQuestions.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500">
                {language === 'en' ? "No matches found for your search." : "No se encontraron coincidencias para tu búsqueda."}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              ...
              <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <label className="flex items-center gap-3 p-4 bg-[#1a1a1a] rounded-xl border border-gray-800 cursor-pointer hover:border-purple-500/50 transition-colors group">
                    <input type="checkbox" checked={hasExplanation} onChange={(e) => setHasExplanation(e.target.checked)} className="w-5 h-5 rounded border-gray-700 bg-black text-purple-600 focus:ring-purple-500 focus:ring-offset-0" />
                  ...
                  <label className="flex items-center gap-3 p-4 bg-[#1a1a1a] rounded-xl border border-gray-800 cursor-pointer hover:border-purple-500/50 transition-colors group">
                    <input type="checkbox" checked={hasSpanish} onChange={(e) => setHasSpanish(e.target.checked)} className="w-5 h-5 rounded border-gray-700 bg-black text-purple-600 focus:ring-purple-500 focus:ring-offset-0" />
                    <div className="flex flex-col">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input type="file" accept=".txt" onChange={(e) => setEnglishFile(e.target.files?.[0] || null)} className="hidden" id="en-file" />
                      <label htmlFor="en-file" className={`flex items-center justify-between p-4 rounded-xl border-2 border-dashed transition-all cursor-pointer ${englishFile ? 'border-purple-500 bg-purple-500/5' : 'border-gray-800 bg-black/20 hover:border-gray-700'}`}>
                        <div className="flex items-center gap-3">
                          <FileText className={`w-5 h-5 ${englishFile ? 'text-purple-500' : 'text-gray-600'}`} />
                          <span className={`text-sm ${englishFile ? 'text-gray-200' : 'text-gray-500'}`}>{englishFile ? englishFile.name : "Select ingles.txt"}</span>
                        </div>
                  ...
                  {hasSpanish && (
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <input type="file" accept=".txt" onChange={(e) => setSpanishFile(e.target.files?.[0] || null)} className="hidden" id="es-file" />
                        <label htmlFor="es-file" className={`flex items-center justify-between p-4 rounded-xl border-2 border-dashed transition-all cursor-pointer ${spanishFile ? 'border-purple-500 bg-purple-500/5' : 'border-gray-800 bg-black/20 hover:border-gray-700'}`}>
                          <div className="flex items-center gap-3">
                            <FileText className={`w-5 h-5 ${spanishFile ? 'text-purple-500' : 'text-gray-600'}`} />
                            <span className={`text-sm ${spanishFile ? 'text-gray-200' : 'text-gray-500'}`}>{spanishFile ? spanishFile.name : "Select español.txt"}</span>
                          </div>
              ...
              <div className="p-6 bg-[#1a1a1a] border-t border-gray-800 flex gap-3">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 py-3 rounded-xl border border-gray-800 text-gray-400 font-bold hover:bg-gray-800 transition-all">
                  CANCEL
                </button>
                <button onClick={handleUpload} className="flex-1 py-3 rounded-xl bg-purple-600 text-white font-bold hover:bg-purple-500 shadow-lg shadow-purple-900/20 transition-all">
                  LOAD DATA
                </button>
              </div>
            </motion.div>
